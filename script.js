...
function createPositionCard(position) {
    ...
    if (position.comparisonMode) {
        ...
        // Calculate client rate based on HIGHEST cost location at desired margin
        // This gives us a standard rate to compare all locations against
        const highestCost = Math.max(...locations.map(l => l.cost));
        const standardClientRate = highestCost / (1 - position.desiredMargin);
        
        locations.forEach(loc => {
            loc.clientRate = standardClientRate; // Same rate for all!
            loc.totalCost = position.hours * loc.clientRate;
            loc.totalProfit = loc.totalCost - (position.hours * loc.cost);
            loc.actualMargin = ((loc.clientRate - loc.cost) / loc.clientRate) * 100; // Actual margin achieved!
        });
        
        locations.sort((a, b) => b.actualMargin - a.actualMargin); // Sort by best margin
        ...
        card.innerHTML = `
            ...
            <div class="all-locations-comparison">
                <div class="comparison-header">
                    ...
                    <span class="comparison-title">If You Charge Client ${formatCurrency(standardClientRate)}/hr - Which Location Wins?</span>
                </div>
